...
**本手结算后，YOU 的筹码为 0 就结束游戏**，不再发下一手或请求对手行动；手动、托管、观战和无界面模式都适用。全屏界面保留最后一手的亮牌结果，显示「你的筹码已归零」，按 Q / Enter 退出。全押后暂时没有剩余筹码时，仍会正常完成发牌和结算；赢回筹码就能继续玩。

普通对局直到你出局、只剩一人，或达到 `--hands` 指定手数。筹码为零的对手座位自动跳过；退出后不保存进度，不包含联网真人多人模式、充值或提现。
